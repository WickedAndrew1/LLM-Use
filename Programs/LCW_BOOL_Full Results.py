...
import pandas as pd
e=""
#Use actual file directory that connects to the folder where it has all the CSV files from deepeval.
...
#Somewhat lazy; I did not feel like creating one individual row for total number of passes, coherency, accuracy, and relevancy. It's inputted for each row.
    d.append(row)
odf=pd.DataFrame(d)
filename=os.path.join(ee,"LCW-Bool3-Full Trial 6 Results.csv")
